...
<table>
  <tr>
    <td><img src="docs/belt-outside.jpg" alt="Outside of the chest belt, with the Movesense MD sensor clipped into it" width="390"></td>
    <td><img src="docs/belt-inside.jpg" alt="Inside of the same belt, showing the two ECG electrode pads" width="390"></td>
  </tr>
</table>
...
- **macOS (Apple Silicon included, e.g. M1 to M3):** fully supported, no
  Rosetta or extra setup needed, because bleak's CoreBluetooth backend
  is native arm64. Two Mac-specific things to know:
...
- **Running from an editor's integrated terminal:** works the same as
  running it from a plain shell, with the same `pip install` and
...
- **Battery level** (`/System/Energy/Level`) was tried and pulled back
  ...
  GSP command set (just HELLO/SUBSCRIBE/UNSUBSCRIBE), and the battery
  percentage may simply not change during a normal session.
